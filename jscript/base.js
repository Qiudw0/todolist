...
Base.prototype.addRule = function (num, selectorText, cssText, position) {
	/*
	*@fun: 动态添加link或style的CSS规则
	*@param: num: 需要添加规则的样式文件(当加载了多个css文件或style标签的时候)；
	*    selectorText: 需要添加规则的选择器(比如body或某个div)；cssText: CSS样式；
	*    position: 添加的规则的在该样式文件中的位置(如添加在开头位置)；
	*@return: 返回Base对象
	*/
...
Base.prototype.removeRule = function (num, index) {
	/*
	*@fun: 动态移除link或style的CSS规则
	*@param: num: 需要移除规则的样式文件；index: 规则在样式文件中的位置
	*@return: 返回Base对象
	*/
	var sheet = document.styleSheets[num];
