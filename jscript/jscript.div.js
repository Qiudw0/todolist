...
// 定义纵向滚动的函数
jscript.div.scrollH = function(len,number,  step,  time){
    /*
    *@fun: 实现纵向滚动
    *@param: len: 滚动的总距离；number: 每一次滚动的距离；step: 每次滚动距离的步增值
    * time: 每次滚动的延迟时间
    */
    ...
    if(number < len){
        ...
        number += step;
        setTimeout("jscript.div.scrollH(" + len + ", " + number + ", " + step + ", " + time + ")");
    }else{
        number = len;
...
jscript.div.scrollW = function(id, firstML, len, count, step, time){
    /*
    *@fun: 实现横向滚动
    *@param: id: 被滚动对象的id；firstML: 被滚动对象的初始marginLeft值；len: 滚动的总距离；
    *    count: 用于累加每次滚动的距离，以判断是否停止滚动；step: 滚动的步增值；time: 完成
    *    滚动所需的时间
    */

    var elem = this.getElement(id);
    var number = elem.offsetLeft;  // 当前marginLeft

    if (count < Math.abs(len)) {
        count += Math.abs(step);
        number += step;
        if(step > 0){  // 向右移动
            if(number < (firstML + len)){
                elem.style.marginLeft = number + 'px';
            }
        }else{  // 向左移动
            if(Math.abs(number) < Math.abs(firstML + len)){
                elem.style.marginLeft = number + 'px';
            }
        }

        setTimeout("jscript.div.scrollW('" +id + "', " + firstML + ", " + len + ", " + count + ", " + step + ", " + time + ")");
    }else{
        number = firstML + len;
        elem.style.marginLeft = number + 'px';
    }
